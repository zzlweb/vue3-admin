...
class Container extends Component {
    ...
    setWidth = (e) => {
      let v = this.positiveNumber(e.target.value); const min = 1
      if (v < min) v = min
    ...
    setQuadraticPosition = (coord, e) => {
      const coords = this.state.points[this.state.activePoint].q
      let v = this.positiveNumber(e.target.value)
    ...
    reset = (e) => {
      const w = this.state.w; const h = this.state.h
...
function Choices (props) {
  const choices = props.choices.map((c, i) => {
    return (
